...
public class RecipeController {
    ...
    public String addNewRecipe(@RequestBody Recipe recipe) {
        return dao.addRecipe(recipe.getEmail(), recipe.getRecipename(), recipe.getCategory(),
    ...
    public boolean updateOldRecipe(@RequestBody Recipe recipe){
        return dao.updateRecipe(recipe.getRecipeid(),recipe.getRecipetext(), recipe.getRecipename(), recipe.getCategory(), recipe.getIngredients(), recipe.getPreparation(), recipe.getCooking());
    }

    @CrossOrigin(origins = URL)
    @RequestMapping(path = "/recentRecipes", method = RequestMethod.POST, consumes = "application/json")
    public boolean addRecentRecipe(@RequestBody RecentRecipe recipe) {
        return dao.addRecentRecipe(recipe.getEmail(), recipe.getRecipeid());
    }
    ...
    public ArrayList<RecentRecipe> getRecentRecipes(@RequestParam(value = "email") String email) {
        return dao.getRecentRecipes(email);
    }
}
